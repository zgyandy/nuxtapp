  ...
  <div class="videoPlay">
    ...
    <dl class="lecturer dis-flex-bt">
      <dt @click="showDialogs">
          <img :src="teacher.id * 1 == -1 ? 'http://static.zhizuobiao.com/images/video/iotek.png' : teacher.photo" alt="头像">
          <div class="msg">
    ...
    <ul class="videoList">
      <li class="dis-flex-bt" v-for="(item, key) in pagedResultArr" :key="key">
        <a :href="$store.state.domain+ '/video/play-' + item.id + '.htm/'" class="dis-flex-bt">
          <div class="msg">
    ...
    <div class="hide overflowHide" v-show="dialog">
      <div class="planning" @click="clickHide">
        <div class="planer" @click.stop="stopClick">
...
export default {
  ...
  created () {
    // 检测是否登录
    this.$get(this.$store.state.domain + '/api/queryLoginUser').then(res => {
  ...
  methods: {
    getVideo () {
      // 视频信息
      this.$get(this.$store.state.domain + "/api/play/video", {videoId: parseInt(this.$route.params.id)}).then(res => {
        if (res.data.code === 200) {
          ...
          this.rangeDataId = res.data.data.rangeDataId
          ...
          if(!this.cards){
              ...
              this.$jsonp(_this.$store.state.forumCode+'/sendMessages/validMobileCode',mobileCode).then(json=>{
                  if(json.code==2){
                      ...
                      //此处直接执行表单提交
                      this.$post(this.$store.state.domain + '/api/user/save',dataOrder).then(res=>{
                          this.dislod = true;
                          const loca = this;
                          const interval = window.setInterval(function () {
                              loca.dialog = false;
    ...
    teacherMsg (n) {
      this.$get(this.$store.state.domain + '/api/play/teacher', {videoId: parseInt(this.$route.params.id), start: n ? n : this.pagedPage}).then(res => {
        if (res.data.code === 200) {
          ...
          if (n) {
            this.pagedResultAr = this.pagedResultArr.concat(res.data.data.pagedResult.dataList)
          }else {
    ...
    commentFn (n) {
      this.$get(this.$store.state.domain + '/api/play/comments',{videoId: parseInt(this.$route.params.id), page: n}).then(res => {
        if (res.data.code === 200) {
    ...
    sendComment () {
      if (this.isLogin) {
        // 评论
        if (this.comment) {
          this.$post(this.$store.state.domain + '/api/play/saveComment', {videoId: parseInt(this.$route.params.id), content: this.comment, 
    ...
    showDialogs(){
        if (~this.teacher.id){
            location.assign(this.$store.state.domain + '/blog/teacher' + this.teacher.no + '/');
        }else{
            location.assign(this.$store.state.domain+'/about/us')
        }
    }
  },
  beforeDestroy () {
