...
export default {
  ...
  methods: {
    search (n, num) {
      this.$get('/api/video/search?',{condition: n, key: this.key, start: num ? num : this.start, limit: 10}).then((res) => {
        if (n == 1) {
          // 视屏搜索
          if (num) {
            this.videoArr = res.data.data.dataList.concat(this.videoArr)
          }else {
          ...
          // 文章搜索
          if (num) {
            this.articleArr = res.data.data.dataList.concat(this.articleArr)
          }else {
